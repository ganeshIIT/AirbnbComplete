...
def full_load(df, tbl, hasindex=True, custom={"id": "INT PRIMARY KEY"}):
    ...
    list_df = np.array_split(df, len(df)//50000 + 1)
    with pyodbc.connect(helper.get_connstring()) as conn:
        helper.to_sqlserver(
            df=list_df[0], name=tbl, conn=conn, if_exists="replace", custom=custom, temp=False)
        conn.commit()
        for i in range(1, len(list_df)):
            helper.to_sqlserver(
                df=list_df[i], name=tbl, conn=conn, if_exists="append", temp=False)
            conn.commit()
    conn.close()
